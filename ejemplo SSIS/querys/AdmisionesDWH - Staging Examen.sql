...
--identificar cuantos registros he tranferrido cada vez que actualizo mi tabla de hehcos
--identifcar: campo fecha se va usar para visualizar la ultima fecha que se trajo un registros, ultima fecha de actualizacion o modificacion que venian de los datos fuentes
--esto sirve para poder traer la data nueva por ejemplo las ultimas 24 horas de informacion no todos los años ya que si se trae siempre todo es un proceso ineficiente
...
--este caampo nuevo de id batch en el fact ayuda para saber en que batch vino el registo para porcesos de auditorio
...
--Query para llenar datos en Staging es si esta en el mismo server
SELECT r.ID_Examen, 
       r.ID_Candidato, 
       r.ID_Carrera, 
       r.ID_Descuento, 
...
--los signos de interrogacion no son para managment studio sino para ssis para parametrizar un valor, recordar crear la fechamodificacion en la tabla examen de la base admisiones
go
---si esta en distinto server se debe de colocar la conexion al server y quitar el admisiones
SELECT r.ID_Examen, 
       r.ID_Candidato, 
       r.ID_Carrera, 
       r.ID_Descuento, 
